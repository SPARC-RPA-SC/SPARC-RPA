 ...
 *          the multiplication together. TODO: think of a more efficient way!
 */
void Hamiltonian_vectors_mult_filteringBug(
    const SPARC_OBJ *pSPARC, int DMnd, int *DMVertices, double *Veff_loc,
    ATOM_NLOC_INFLUENCE_OBJ *Atom_Influence_nloc, NLOC_PROJ_OBJ *nlocProj, 
    int ncol, double c, double *x, double *Hx, int spin, MPI_Comm comm
)
{
    ...
    int nproc;
    MPI_Comm_size(comm, &nproc);
    int commRank, blacsRank;
    MPI_Comm_rank(comm, &commRank); 
    MPI_Comm_rank(pSPARC->blacscomm, &blacsRank);
    double t1, t2;
    MPI_Barrier(comm); 
    MPI_Barrier(pSPARC->blacscomm);
    t1 = MPI_Wtime();
    // TODO: make dims an input parameter
    int dims[3], periods[3], my_coords[3];
    if (nproc > 1)
    ...
    MPI_Barrier(pSPARC->blacscomm);
    t2 = MPI_Wtime();
    double bandComm_tLx, general_tLx;
    bandComm_tLx = (t2 - t1) * 1e3;
    // MPI_Reduce(&cumStep2Hamil, &bandComm_cumStep2Hamil, 1, MPI_DOUBLE, MPI_MAX, 0, pSPARC->bandcomm);
    MPI_Reduce(&bandComm_tLx, &general_tLx, 1, MPI_DOUBLE, MPI_MAX, 0, pSPARC->blacscomm);
    if (!(commRank || blacsRank)) printf("the max time of all processors spent on Lx is %.12f ms\n", general_tLx);
    // adding Exact Exchange potential  
    if ((pSPARC->usefock > 0) && (pSPARC->usefock % 2 == 0)){
        exact_exchange_potential((SPARC_OBJ *)pSPARC, x, ncol, DMnd, Hx, spin, comm);
    }
    ...
    MPI_Barrier(pSPARC->blacscomm);
    t2 = MPI_Wtime();
    double bandComm_tVnlx, general_tVnlx;
    bandComm_tVnlx = (t2 - t1) * 1e3;
    MPI_Reduce(&bandComm_tVnlx, &general_tVnlx, 1, MPI_DOUBLE, MPI_MAX, 0, pSPARC->blacscomm);
    if (!(commRank || blacsRank)) printf("the max time of all processors spent on Vnl*x is %.12f ms\n", general_tVnlx);
}
